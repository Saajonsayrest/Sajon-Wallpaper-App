import 'package:flutter/widgets.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';

/// Defines the text styles for the app.
abstract class AppTextStyle {
  ///Bold Text 20sp
  static TextStyle boldText20 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w700,
    fontSize: 20.sp,
  );
  static TextStyle boldText24 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w700,
    fontSize: 24.sp,
  );
  static TextStyle boldText94 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w700,
    fontSize: 94.sp,
  );

  static TextStyle boldText40 = TextStyle(
      fontFamily: 'PoppinsRegular',
      fontWeight: FontWeight.w700,
      fontSize: 40.sp,
      letterSpacing: 0);

  static TextStyle boldText38 = TextStyle(
      fontFamily: 'PoppinsRegular',
      fontWeight: FontWeight.w700,
      fontSize: 38.sp,
      letterSpacing: 0);

  static TextStyle boldText34 = TextStyle(
      fontFamily: 'PoppinsRegular',
      fontWeight: FontWeight.w700,
      fontSize: 34.sp,
      letterSpacing: 0);
  static TextStyle boldText30 = TextStyle(
      fontFamily: 'PoppinsRegular',
      fontWeight: FontWeight.w700,
      fontSize: 30.sp,
      letterSpacing: 0);

  ///Bold Text 16sp
  static TextStyle boldText16 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w700,
    fontSize: 16.sp,
  );

  ///Bold Text 14sp
  static TextStyle boldText14 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w700,
    fontSize: 14.sp,
  );
  static TextStyle boldText12 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w700,
    fontSize: 12.sp,
  );

  ///SemiBold Text 18sp
  static TextStyle semiBoldText20 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w600,
    fontSize: 20.sp,
  );

  ///SemiBold Text 18sp
  static TextStyle semiBoldText18 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w600,
    fontSize: 18.sp,
  );

  ///SemiBold Text 16sp
  static TextStyle semiBoldText16 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w600,
    fontSize: 16.sp,
  );

  ///SemiBold Text 14sp
  static TextStyle semiBoldText14 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w600,
    fontSize: 14.sp,
  );

  ///SemiBold Text 12sp
  static TextStyle semiBoldText12 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w600,
    fontSize: 12.sp,
  );

  ///Medium Text 20sp
  static TextStyle mediumText20 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w500,
    fontSize: 20.sp,
  );

  ///Medium Text 24sp
  static TextStyle mediumText24 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w500,
    fontSize: 24.sp,
  );

  ///Medium Text 26sp
  static TextStyle mediumText26 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w500,
    fontSize: 26.sp,
  );
  static TextStyle mediumText30 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w500,
    fontSize: 30.sp,
  );

  ///Medium Text 18sp
  static TextStyle mediumText18 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w500,
    fontSize: 18.sp,
  );

  ///Medium Text 19sp
  static TextStyle mediumText19 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w500,
    fontSize: 19.sp,
  );

  ///Medium Text 16sp
  static TextStyle mediumText16 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w500,
    fontSize: 16.sp,
  );

  ///Medium Text 14sp
  static TextStyle mediumText14 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w500,
    fontSize: 14.sp,
  );

  ///Medium Text 12sp
  static TextStyle mediumText12 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w500,
    fontSize: 12.sp,
  );

  static TextStyle mediumText11 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w500,
    fontSize: 11.sp,
  );
  static TextStyle mediumText13 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w500,
    fontSize: 13.sp,
  );

  ///Medium Text 40sp
  static TextStyle regularText40 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 40.sp,
    letterSpacing: 1.w,
    height: 1.2,
  );
  static TextStyle regularText42 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 42.sp,
    letterSpacing: 1.w,
    height: 1.2,
  );

  ///Medium Text 60sp
  static TextStyle regularText60 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 60.sp,
    letterSpacing: 1.w,
    height: 1.2,
  );

  ///Medium Text 60sp
  static TextStyle regularText30 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 30.sp,
    letterSpacing: 1.w,
    height: 1.133,
  );

  static TextStyle regularText24 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 24.sp,
    letterSpacing: 1.w,
    height: 1.2,
  );

  static TextStyle regularText28 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 28.sp,
    height: 1.2,
  );
  static TextStyle regularText23 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 23.sp,
    letterSpacing: 1.w,
    height: 1.2,
  );

  static TextStyle regularText20 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 20.sp,
  );

  ///Medium Text 18sp
  static TextStyle regularText18 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 18.sp,
  );

  ///Medium Text 14sp
  static TextStyle regularText16 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 16.sp,
  );

  ///Medium Text 14sp
  static TextStyle regularText14 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 14.sp,
  );
  static TextStyle regularText15 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 15.sp,
  );
  static TextStyle regularText13 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 13.sp,
  );

  ///Medium Text 12sp
  static TextStyle regularText12 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 12.sp,
  );

  ///Medium Text 10sp
  static TextStyle regularText10 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 10.sp,
  );

  static TextStyle regularText11 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w400,
    fontSize: 11.sp,
  );

  ///Medium Text 12sp
  static TextStyle lightText12 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w300,
    fontSize: 12.sp,
  );

  static TextStyle lightText14 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w300,
    fontSize: 14.sp,
  );
  static TextStyle lightText13 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w300,
    fontSize: 13.sp,
  );

  static TextStyle lightText15 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w300,
    fontSize: 14.sp,
  );

  static TextStyle lightText10 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w300,
    fontSize: 10.sp,
  );
  static TextStyle lightText29 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w300,
    fontSize: 29.sp,
  );
  static TextStyle lightText26 = TextStyle(
    fontFamily: 'PoppinsRegular',
    fontWeight: FontWeight.w300,
    fontSize: 26.sp,
  );
}
